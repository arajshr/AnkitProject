...
public class Denial_DOS extends Configuration
{
	...
	public void verify_Denail_DOS()
	{
		...
		objTransactionInbox.click_Image_Denail(false);
		logger.log(LogStatus.INFO, "Navigate to DENIAL queue");
		...
		int accountCount = objTransactionInbox.get_TotalAccounts(Constants.Queues.DENIAL);		
		if(accountCount > 0)
		{
			...
			{
				String accountNumber = objTransactionInbox.get_AccountNumber(Constants.Queues.DENIAL, i);
				//Configuration.logger.log(LogStatus.INFO, "Account Number: " + accountNumber);
				
				if(objTransactionInbox.open_PatientAccount(Constants.Queues.DENIAL, i).getStatus())
				{
...
//				new WebDriverWait(driver, 5).until(ExpectedConditions.invisibilityOfElementLocated(By.xpath("//div[@class='modal-backdrop fade']")));
			...
			Configuration.logger.log(LogStatus.INFO, "<b>No accounts in DENIAL queue under DOS 120+</b>");
	

		objTransactionInbox.click_Aging_Category_91_120();
		logger.log(LogStatus.INFO, "Click DOS Aging 91-120");
		
		int accountCount_91_120 = objTransactionInbox.get_TotalAccounts(Constants.Queues.DENIAL);
		if(accountCount_91_120 > 0)
		{
			...
			Configuration.logger.log(LogStatus.INFO, "<b>No accounts in DENIAL queue under DOS 91-120</b>");
		...
		int accountCount_61_90 = objTransactionInbox.get_TotalAccounts(Constants.Queues.DENIAL);
		...
		if(accountCount_61_90 > 0)
		{
			...
			Configuration.logger.log(LogStatus.INFO, "<b>No accounts in DENIAL queue under DOS 61-90</b>");
		...
		int accountCount_31_60 = objTransactionInbox.get_TotalAccounts(Constants.Queues.DENIAL);
		...
		if(accountCount_31_60 > 0)
		{
			...
			Configuration.logger.log(LogStatus.INFO, "<b>No accounts in DENIAL queue under DOS 31-60</b>");
		
		
		objTransactionInbox.click_Aging_Category_0_30();
		logger.log(LogStatus.INFO, "Click DOS Aging 0-30");
		
		int accountCount_0_30 = objTransactionInbox.get_TotalAccounts(Constants.Queues.DENIAL);
		...
		if(accountCount_0_30 > 0)
		{
			...
			Configuration.logger.log(LogStatus.INFO, "<b>No accounts in DENIAL queue under DOS 0-30</b>");
	}
}
